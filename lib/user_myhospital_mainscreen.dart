...
class _UserMyHospitalMainScreenState extends State<UserMyHospitalMainScreen> {
  ...
  Future<void> _loadHospitalNotice() async {
    ...
    try {
      final uri = Uri.parse(
          '$_baseUrl/api/hospitals/${widget.hospitalId}/notice');
      ...
      if (res.statusCode == 200) {
        ...
        final noticeText =
        (data['notice'] ?? data['message'] ?? '').toString();
        if (mounted) {
          setState(() {
            _notice = noticeText.trim().isEmpty
                ? '공지 없음'
                : noticeText.trim();
            _loading = false;
          });
    ...
    try {
      ...
      if (res.statusCode == 200) {
        ...
        final c = (body is Map && body['count'] is num)
            ...
            : 0;
        setState(() => _unreadNotificationCount = c);
      } else {
  ...
  // 💬 미확인 채팅 카운트
  // 👉 서버에 /api/hospitals/:hospitalId/chat/unread-count 엔드포인트 맞춰줘야 함
  Future<void> _loadChatUnreadCount() async {
    try {
      final uri = Uri.parse(
          '$_baseUrl/api/hospitals/${widget.hospitalId}/chat/unread-count');
      final res = await _http
          .get(uri, headers: {'Authorization': 'Bearer ${widget.token}'})
          .timeout(_timeout);
      if (!mounted) return;
  ...
  Future<void> _loadPetPreview() async {
    ...
    try {
      ...
      if (res.statusCode == 200) {
        ...
        final parsed = list
            .map((e) =>
            _PetPreview.fromJson((e as Map).cast<String, dynamic>()))
            .toList();
        if (!mounted) return;
        setState(() {
          _petPreview = parsed;
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
        ...
        child: SafeArea(
          child: ListView(
            children: [
              ...
              const _DrawerTile(
                  icon: Icons.settings_outlined, title: '설정'),
            ],
          ),
        ),
      ),
      ...
      floatingActionButton: _ChatBubbleFab(
        unreadCount: _unreadChatCount, // 💬 채팅용 카운트만 사용
        onTap: _openChat,
      ),
      ...
      bottomNavigationBar: BottomNavigationBar(
        ...
        onTap: (i) {
          switch (i) {
            ...
            case 1:
              _noAnimReplace(
                  HealthDashboardScreen(token: widget.token));
              break;
            case 2:
            // 현재 화면
              break;
  ...
  Future<bool> _deleteAppt(String apptId) async {
    Future<http.Response> _try(String path,
        {bool withHospital = false}) {
...
class _Appt {
  ...
  factory _Appt.fromJson(Map<String, dynamic> m) {
    ...
    if (dt == null) {
      ...
      final parsed =
      raw.isNotEmpty ? DateTime.tryParse(raw) : null;
      if (parsed != null) {
    ...
    return _Appt(
      ...
      doctor:
      (m['doctorName'] ?? m['doctor'] ?? '의사').toString(),
      ...
      userName: _clean(
          (m['userName'] ??
              ...
              ?.toString()),
      petName:
      _clean((m['petName'] ?? m['pet'])?.toString()),
    );
  }
...
class _PetPreview {
  ...
  factory _PetPreview.fromJson(Map<String, dynamic> m) {
    ...
    if (img.isEmpty &&
        m['images'] is List &&
        (m['images'] as List).isNotEmpty) {
      img = ((m['images'] as List).first ?? '')
          .toString();
    }
...
class _MainDaySheet extends StatefulWidget {
  ...
  @override
  State<_MainDaySheet> createState() =>
      _MainDaySheetState();
}
            ...
            if (widget.items.isEmpty)
              ...
              ...widget.items.expand((a) {
                final expanded =
                    _expandedId == a.id;
                final label =
                statusLabelForUser(a.status);

                final who = [
                  if ((a.userName ?? '').isNotEmpty)
                    a.userName!,
                  if ((a.petName ?? '').isNotEmpty)
                    a.petName!,
                  if (a.doctor.isNotEmpty)
                    a.doctor,
                ].join(' / ');
...
class _ApprovedOnlyNotice extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final approvedUpcoming = apptsByDate.values
        .expand((e) => e)
        .where((a) =>
    _isApproved(a.status) &&
        a.visit.isAfter(now))
        .toList()
      ..sort((a, b) =>
          a.visit.compareTo(b.visit));
...
class _ScheduleMemoCalendar
  ...
  final void Function(DateTime, List<_Appt>)
  onTapDay;
  ...
  Widget build(BuildContext context) {
    ...
    final first =
    DateTime(ym.year, ym.month, 1);
    final daysInMonth =
        DateTime(ym.year, ym.month + 1, 0)
            .day;

    final firstWeekday = first.weekday; // 1=Mon…7=Sun
    final leading =
        (firstWeekday + 6) % 7; // Mon:0 … Sun:6

    final totalCells =
        leading + daysInMonth;
    final rows =
    ((totalCells + 6) ~/ 7).clamp(5, 6);
...
class _CalendarCell extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final date = DateTime(
        ym.year, ym.month, dayNum);
    final key =
        '${date.year}-${date.month.toString().padLeft(2, '0')}-${date.day.toString().padLeft(2, '0')}';
    final list =
        apptsByDate[key] ?? const <_Appt>[];
...
class _BannerNotice extends StatelessWidget {
  const _BannerNotice(
      {required this.loading, required this.text});
  final bool loading;
  final String text;
...
class _PreviewCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final date =
    _fmtDateDot(preview.createdAt);
    final time =
    _fmtKTime(preview.createdAt);
